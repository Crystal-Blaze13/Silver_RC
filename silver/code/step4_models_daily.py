...
class Config:
    ...
    IMF_FILE        = "../data/imfs_daily.npy"
    COMPLEXITY_FILE = "../data/imf_complexity_daily.csv"
    LASSO_FILE      = "../data/lasso_selected_features_daily.pkl"
...
def main():
    ...
    # ── Load ──────────────────────────────────────────────────────────────
    silver_df    = pd.read_csv(Config.SILVER_FILE, index_col=0, parse_dates=True)
    silver_price = silver_df.iloc[:,0].values
    dates        = silver_df.index

    u_sorted = np.load(Config.IMF_FILE)
    _np, _ni = len(silver_price), u_sorted.shape[1]
    if _ni != _np:
        trim = _ni - _np
    ...
    y_true_train = silver_price[N_LAGS:n_train]
    test_dates   = dates[n_train:]

    # Silver log returns (needed for single-model target)
    silver_logret = np.concatenate([[0.0],
        np.log(silver_price[1:] / silver_price[:-1])])
    # logret[t] = log(price[t]/price[t-1]), aligned with silver_price index

    recon   = u_sorted[:,N_LAGS:].sum(axis=0)
    max_err = float(np.max(np.abs(recon - silver_price[N_LAGS:]))) \
        if len(recon)==len(silver_price)-N_LAGS else np.nan
    ...
    y_logret_all = silver_logret[N_LAGS:]   # length = len(X_full)
    ...
    price_before_test = float(silver_price[n_train - 1])
    ...
    table7 = pd.concat([table7,
        ...
        ignore_index=True)
    table7.to_csv("../results/tables/table7_single_model_errors.csv", index=False)
    print("\nTable 7:\n", table7.to_string(index=False))

    table8 = pd.DataFrame(decomp_metrics).T.reset_index()
    table8.columns = cols
    table8.to_csv("../results/tables/table8_decomp_model_errors.csv", index=False)
    ...
    plt.tight_layout()
    plt.savefig("../results/figures/fig9_single_model_forecasts.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig("../results/figures/fig10_error_barplots.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    with open("../data/predictions_daily.pkl","wb") as f:
        pickle.dump({
    ...
    print("Saved: predictions_daily.pkl")
    print("\n"+"="*65)
    print("STEP 4 COMPLETE [v5]")
    print("NEXT: python step5_dmtest_daily.py")
    progress("Done")
