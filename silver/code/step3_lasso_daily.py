...
OUT_TABLE6 = "../results/tables/table6_lasso_features.csv"
OUT_PKL    = "../data/lasso_selected_features_daily.pkl"
...
silver_s = df["mcx_silver"]

# ── 2a. Log-return lags of silver ─────────────────────────────────────────────
silver_logret = np.log(silver_s / silver_s.shift(1))
...
df["silver_ret5"]  = silver_s.pct_change(5).shift(1)
df["silver_ret20"] = silver_s.pct_change(20).shift(1)
df["silver_ret60"] = silver_s.pct_change(60).shift(1)
...
df["silver_rsi14"]    = _rsi(silver_s, 14).shift(1)
df["silver_ma_ratio"] = (silver_s / silver_s.rolling(20).mean()).shift(1)
df["silver_vol10"]    = (silver_s.pct_change().rolling(10).std().shift(1)
                         * np.sqrt(252) * 100)

MOMENTUM_FEATURES = ["silver_ret5", "silver_ret20", "silver_ret60",
                     "silver_rsi14", "silver_ma_ratio", "silver_vol10"]
if gold_col:
...
# ── 3. Build X_full ───────────────────────────────────────────────────────────
silver_arr = silver_logret.values
n_obs      = len(df)

lag_names         = [f"silver_logret_lag_{i}" for i in range(1, N_LAGS + 1)]
...
print(f"\nFeature breakdown:")
print(f"  Log-return lags (silver):   {len(lag_names)}")
...
for t in range(N_LAGS, n_obs):
    lr_lags = [silver_arr[t - i] for i in range(1, N_LAGS + 1)]
...
lag_sel = sum(1 for i in range(K)
              ...
              if f.startswith("silver_logret_lag"))
ext_sel = sum(1 for i in range(K)
              for f in selected_features[i]
              if "_logret_lag1" in f and not f.startswith("silver"))
...
print("=" * 65)
print("NEXT: python step4_models_daily.py")