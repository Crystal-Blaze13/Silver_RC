"""
STEP 1 (daily) — VMD Decomposition of MCX Gold Price
=======================================================
...
Produces:
  fig4_gold_price_split.png
  fig7_gold_imf_decomposition.png
  table5_gold_imf_statistics.csv
  imfs_daily_gold.npy          ← used by steps 2–6
  financial_data/processed/gold_daily.csv
  financial_data/processed/n_train_daily_gold.npy
...
OUT_FIG4   = "../results/figures/fig4_gold_price_split.png"
OUT_FIG7   = "../results/figures/fig7_gold_imf_decomposition.png"
OUT_TAB5   = "../results/tables/table5_gold_imf_statistics.csv"
OUT_IMFS   = "../data/imfs_daily_gold.npy"
OUT_GOLD = "../data/gold_daily.csv"
OUT_NTRAIN = "../data/n_train_daily_gold.npy"
...
if GOLD_COL not in df.columns:
    raise ValueError(f"Column '{GOLD_COL}' not found. "
                     f"Available: {list(df.columns)}")

price = df[GOLD_COL].dropna()
print(f"Loaded {len(price)} daily observations")
print(f"Date range : {price.index[0].date()} → {price.index[-1].date()}")
print(f"Price range: {price.min():,.0f} – {price.max():,.0f} INR/10g")

os.makedirs(os.path.dirname(OUT_GOLD), exist_ok=True)
price.to_csv(OUT_GOLD, header=True)
...
print(f"Max reconstruction error: {recon_err:.4f} INR/10g "
...
print(f"Final reconstruction error: {final_err:.4f} INR/10g")
...
ax.set_title("MCX Gold Price — Daily (INR/10g)", fontsize=13, fontweight="bold")
ax.set_xlabel("Date")
ax.set_ylabel("Price (INR/10g)")
...
fig.suptitle(f"VMD Decomposed IMFs (K={K}) — MCX Gold Daily (INR/10g)",
...
print(f"  {OUT_GOLD}  → Silver series for Steps 2–6")
print(f"  {OUT_NTRAIN}  → train split for Steps 3–6")
print("=" * 60)
print("NEXT: python step2_entropy_daily_gold.py")