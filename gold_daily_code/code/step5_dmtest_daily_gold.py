"""
STEP 5 (daily) — Diebold-Mariano Statistical Test
===================================================
Identical logic to step5_dmtest.py; only the input file name changes
(predictions_daily_gold.pkl instead of predictions.pkl).

Input:  predictions_daily_gold.pkl
Output: table9_gold_dm_test.csv
        fig_gold_dm_heatmap.png

HOW TO RUN (from the daily/ directory):
  python step5_dmtest_daily.py
"""
...
OUT_TABLE9 = "../results/tables/table9_gold_dm_test.csv"
OUT_FIG    = "../results/figures/fig_gold_dm_heatmap.png"

# ── 1. Load predictions ───────────────────────────────────────────────────────
print("=" * 60)
print("STEP 5 (daily): Diebold-Mariano Test")
print("=" * 60)
...
def dm_test(y_true, pred1, pred2, h=1):
    """
    Diebold-Mariano with Harvey-Leybourne-Newbold (1997) finite-sample correction.
    Returns dm_stat (positive = pred1 better), p_1sided, p_2sided.
    """
    ...
    dm_stat    = -dm_raw * correction   # positive = model1 better

    df     = n - 1
    p_1sid = 1.0 - stats.t.cdf(dm_stat, df=df)
...
ax.set_title("DM Test Matrix — MCX Gold Forecasting (Daily)\n"
...
print(f"  {OUT_FIG}")
print("=" * 60)
print("NEXT: python step6_trading_daily_gold.py")